/**
 * @file STM32_4kOLED.cpp 
 * @brief SSD1306 OLED Text Display Library for Arduino
 *
...
#include "STM32_4kOLED_Wire.h"

#define SSD1306_COLUMNS 128										///< Default display width in pixels
#define SSD1306_PAGES 4												///< Default display height in pages

#define SSD1306_COMMAND 0x00									///< Command byte
#define SSD1306_DATA 0x40											///< Data byte

// ----------------------------------------------------------------------------

static uint8_t oledOffsetX = 0;								// OLED offset in pixels
static uint8_t oledOffsetY = 0; 							// OLED Offset in pages??

static uint8_t oledWidth = SSD1306_COLUMNS;		// OLED display width in pixels
static uint8_t oledPages = SSD1306_PAGES;			// OLED display height in pages

static const DCUnicodeFont *oledUnicodeFont = 0;
static uint8_t unicodeFontNum = 0;
static const DCfont *oledFont = 0;

static uint8_t oledX = 0;											// Horizontal cursor position in pixels
static uint8_t oledY = 0;											// Vertical cursor position in pages

static uint8_t renderingFrame = 0xB0;					// Default = 0xB0
static uint8_t drawingFrame = 0x40;						// Default = 0x40

static uint8_t doubleSize = 0;								// Double size text? (0=No, 1=Yes)
static uint8_t characterSpacing = 0;					// Text character width (0=proportional font)
...
// Default constructor (uses Wire library)
SSD1306Device::SSD1306Device(void) {
	wireBeginFn = &tiny4koled_begin_wire;
	wireBeginTransmissionFn = &tiny4koled_beginTransmission_wire;
	wireWriteFn = &datacute_write_wire;
	wireEndTransmissionFn = &datacute_endTransmission_wire;
}

// Constructor with initialization to allow for alternative I2C libraries
...
/**
 *  @brief Set font to be used for generating text
 ...
 * 	@param spacing: Text character width (0=proportional font)
 */
...
/**
 *  @brief Get the width in pixels for the specified character
 *  @param c: Character
 *  @returns Width in pixels
 *  @note For fixed fonts, returns the generic width in pixels.
 *        For proportional fonts, it returns the specific width
...
/**
 *  @brief Get the total width in pixels for the specified string with the current font.
...
/**
 * @brief Sets cursor position within the active rendering frame
 * @param x: Column number (0-127)
 * @param y: Page number (0-7)
 * @note X and Y offsets are added to the requested cursor position.
 */
void SSD1306Device::setCursor(uint8_t x, uint8_t y) {
	/*
	 *  Command sequence:
	 *  1. Set GDDRAM page start address (0-7)
	 *  2. Set higher column nibble start address
	 *  3. Set lower column nibble start address
	 */
...
/**
 *  @brief Get the current horizontal "cursor" position in pixels.
...
/**
 *  @brief Get the current vertical "cursor" position in pages (8 pixels).
 *  @returns Vertical position in pages
 */
...
/**
 *  @brief Clear the OLED screen buffer to black
 *  @returns With double-buffering, this only affects the screen buffer 
 *           that we're actively writing to, not the screen buffer
 *           being displayed.
 */
...
/**
 *  @brief Fill the OLED screen buffer with a bit pattern.
 *  @param Fill pattern to be written to the screen buffer.
 *  @returns With double-buffering, this only affects the screen buffer 
 *           that we're actively writing to, not the screen buffer
 *           being displayed.
 */
...
/**
 *  @brief Switch the screen buffer currently being rendered to.
 *  @note Sets the RAM page start address to point to either
 *        the upper (0x00) or lower (0x04) bank of the
 *        controller's display RAM.  Subsequent commands
 * 				will target this RAM buffer.
 */
void SSD1306Device::switchRenderFrame(void) {
	// Set GDDRAM page start address to either
	// 0x00 or 0x04 to select the upper or lower
	// bank of display RAM
	renderingFrame ^= 0x04;
}

/**
 *  @brief Switch the screen buffer currently being displayed.
 *  @note Sets the display start line to either 0x00 or 0x02
 *        to select upper or lower bank of the display RAM.
 */
void SSD1306Device::switchDisplayFrame(void) {
	drawingFrame ^= 0x20;
	// Set display start line to either 0x00 or 0x02
	// to select upper or lower bank of display RAM
	ssd1306_send_command(drawingFrame);
}

/**
 *  @brief Switch BOTH the rendering and display screen buffers.
 */
